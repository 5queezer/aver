...
#[test]
fn extract_python_facts_do_not_treat_generic_base_arguments_as_superclasses() {
    let facts =
        extract_python_facts("store.py", "class Store(BaseStore[Memory]):\n    pass\n").unwrap();
...
#[test]
fn extract_typescript_facts_do_not_split_scoped_interface_extends_reference_into_top_level_interface()
 {
    let facts =
        extract_typescript_facts("memory.ts", "interface Store extends Memory.BaseStore {}")
            .unwrap();

    assert!(facts.contains(&ExtractedFact {
        subject: "Interface:Store".to_string(),
        predicate: "extends".to_string(),
        object: "Interface:Memory.BaseStore".to_string(),
    }));
    assert!(!facts.contains(&ExtractedFact {
        subject: "Interface:Store".to_string(),
        predicate: "extends".to_string(),
...
#[test]
fn extract_java_facts_do_not_treat_generic_type_arguments_as_interface_extends() {
    let facts = extract_java_facts(
        "Recallable.java",
        "interface Recallable extends BaseRecallable<Memory> {}",
    )
...
#[test]
fn extract_ruby_facts_do_not_split_scoped_superclass_references_into_top_level_classes() {
    let facts = extract_ruby_facts(
        "store.rb",
        "class Memory\nend\nclass Store < Memory::BaseStore\nend",
    )
    .unwrap();
...
fn extract_kotlin_facts_emit_multiple_interface_implements_triples() {
    let facts = extract_kotlin_facts(
        "Store.kt",
        "interface Recallable\ninterface Auditable\nclass Store : Recallable, Auditable",
    )
    .unwrap();
...
fn extract_kotlin_facts_emit_object_implements_interface_triple() {
    let facts = extract_kotlin_facts(
        "Store.kt",
        "interface Recallable\nobject Store : Recallable",
    )
    .unwrap();

    assert!(facts.contains(&ExtractedFact {
        subject: "Object:Store".to_string(),
        predicate: "implements".to_string(),
        object: "Interface:Recallable".to_string(),
    }));
}
