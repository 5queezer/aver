...
pub fn answer_prompt(question: &str, _reference_answer: &str, contexts: &[String]) -> String {
    let context_order = if asks_for_current_value(question) {
        "For this question, retrieved memories are ordered newest to oldest."
    } else {
        "For this question, retrieved memories are ordered oldest to newest."
    };
    format!(
