...
fn migration_0009_rejects_out_of_range_confidence_and_invalid_status() {
    ...
    let err = conn
        .execute(
            "INSERT INTO claims (subject, predicate, object, source_refs, provenance, confidence, status, write_ts, created_at, last_seen_at) \
             VALUES ('a', 'rel', 'b', '[\"test_session\"]', 'USER_ASSERTED', 1.5, 'ACTIVE', 1, 1, 1)",
            [],
        )
    ...
    let err = conn
        .execute(
            "INSERT INTO claims (subject, predicate, object, source_refs, provenance, confidence, status, write_ts, created_at, last_seen_at) \
             VALUES ('a', 'rel', 'b', '[\"test_session\"]', 'USER_ASSERTED', -0.1, 'ACTIVE', 1, 1, 1)",
            [],
        )
