...
#[test]
fn entity_type_closure_ancestor_id_must_be_positive() {
    let dir = tempfile::tempdir().unwrap();
    let _store = Store::open(dir.path()).expect("open should succeed");
    drop(_store);
    ...
    let err = conn
        .execute(
            "INSERT INTO entity_type_closure (child_id, ancestor_id) VALUES (?1, 0)",
            [child_id],
        )
        .expect_err("entity type closure ancestor ids must be positive");
    assert!(
        err.to_string()
            .contains("entity_type_closure.ancestor_id must be positive"),
        "unexpected error: {err}"
    );
}
...
#[test]
fn predicate_closure_child_id_must_be_positive() {
    let dir = tempfile::tempdir().unwrap();
    let _store = Store::open(dir.path()).expect("open should succeed");
    drop(_store);
    ...
    let err = conn
        .execute(
            "INSERT INTO predicate_closure (child_id, ancestor_id) VALUES (0, ?1)",
            [ancestor_id],
        )
        .expect_err("predicate closure child ids must be positive");
    assert!(
        err.to_string()
            .contains("predicate_closure.child_id must be positive"),
        "unexpected error: {err}"
    );
}
...
#[test]
fn contradiction_claim_id_must_be_positive() {
    let dir = tempfile::tempdir().unwrap();
    let _store = Store::open(dir.path()).expect("open should succeed");
    drop(_store);
    ...
    let err = conn
        .execute(
            "INSERT INTO contradictions (claim_id, reason, created_at)
             VALUES (0, 'unsupported by source', 1)",
            [],
        )
        .expect_err("contradiction claim ids must be positive");
    assert!(
        err.to_string()
            .contains("contradictions.claim_id must be positive"),
        "unexpected error: {err}"
    );
}
...
#[test]
fn hyperedge_source_refs_must_not_be_empty() {
    let dir = tempfile::tempdir().unwrap();
    let _store = Store::open(dir.path()).expect("open should succeed");
    drop(_store);
    ...
    let err = conn
        .execute(
            "INSERT INTO hyperedges (predicate, provenance, confidence, source_refs, created_at, updated_at)
             VALUES ('depends_on', 'USER_ASSERTED', 1.0, '[]', 1, 1)",
            [],
        )
        .expect_err("hyperedges need at least one source reference");

    assert!(
        err.to_string()
            .contains("hyperedges.source_refs must not be empty"),
        "unexpected error: {err}"
    );
}
...
#[test]
fn observation_prune_marker_ts_must_be_positive() {
    let dir = tempfile::tempdir().unwrap();
    let _store = Store::open(dir.path()).expect("open should succeed");
    drop(_store);
    ...
    let err = conn
        .execute(
            "INSERT INTO observation_prune_markers (id, session_id, pruned_observation_ids, ts)
             VALUES ('marker-1', 'session-1', '[\"obs-1\"]', 0)",
            [],
        )
        .expect_err("observation prune marker timestamps must be positive");

    assert!(
        err.to_string()
            .contains("observation_prune_markers.ts must be positive"),
        "unexpected error: {err}"
    );
}
...
#[test]
fn observation_prune_marker_ids_must_not_be_empty() {
    let dir = tempfile::tempdir().unwrap();
    let _store = Store::open(dir.path()).expect("open should succeed");
    drop(_store);
    ...
    let err = conn
        .execute(
            "INSERT INTO observation_prune_markers (id, session_id, pruned_observation_ids, ts)
             VALUES ('marker-1', 'session-1', '[]', 1)",
            [],
        )
        .expect_err("observation prune marker ids should not be empty");

    assert!(
        err.to_string()
            .contains("observation_prune_markers.pruned_observation_ids must not be empty"),
        "unexpected error: {err}"
    );
}

#[test]
fn observation_prune_marker_session_id_must_not_be_blank() {
    let dir = tempfile::tempdir().unwrap();
    let _store = Store::open(dir.path()).expect("open should succeed");
    drop(_store);
    ...
    let err = conn
        .execute(
            "INSERT INTO observation_prune_markers (id, session_id, pruned_observation_ids, ts)
             VALUES ('marker-1', ' ', '[\"obs-1\"]', 1)",
            [],
        )
        .expect_err("observation prune marker session_id should not be blank");

    assert!(
        err.to_string()
            .contains("observation_prune_markers.session_id must not be blank"),
        "unexpected error: {err}"
    );
}
...
#[test]
fn hyperedge_source_refs_elements_must_be_nonblank_strings() {
    let dir = tempfile::tempdir().unwrap();
    let _store = Store::open(dir.path()).expect("open should succeed");
    drop(_store);
    ...
    let err = conn
        .execute(
            "INSERT INTO hyperedges (predicate, provenance, confidence, source_refs, created_at, updated_at)
             VALUES ('depends_on', 'USER_ASSERTED', 1.0, '[\"session-1\", \" \" ]', 1, 1)",
            [],
        )
        .expect_err("hyperedge source refs should contain nonblank strings");

    assert!(
        err.to_string()
            .contains("hyperedges.source_refs elements must be nonblank strings"),
        "unexpected error: {err}"
    );
}
