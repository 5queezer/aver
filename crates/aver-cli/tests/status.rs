...
//! readiness. This starts the v0.1 `aver status` surface from ADR-0013.

use std::process::Command;

#[test]
fn status_reports_store_ready_for_memory_dir() {
    let dir = std::env::temp_dir().join(format!("aver-cli-status-{}", std::process::id()));
