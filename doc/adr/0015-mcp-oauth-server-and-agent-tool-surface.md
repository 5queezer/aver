...
Aver adds a `aver-server` crate that provides an MCP/OAuth integration layer over `aver-core`.
...
- Configuration via `AVER_*` environment variables.
...
- (+) Claude/Codex can integrate with Aver through standard MCP instead of custom prompt glue.
...
- (+) The event/candidate tools will expose Aver's safer triggered-write architecture instead of encouraging every-turn durable writes.
- (+) Reusing Karta's server shape reduces integration risk while keeping Aver's core memory model independent.
...
1. `crates/aver-server` workspace crate.
2. `ServerConfig::from_env` for `AVER_*` configuration.
3. `AverTools` adapter over `aver-core` with `remember_claim` and `recall`.
...
8. `AverMcpService` using `rmcp` tool macros for `remember_claim` and `recall`.
9. Axum router with discovery route, token route, protected health route, and runnable `aver-server` binary.
